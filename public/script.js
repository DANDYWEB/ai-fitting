...
btn.addEventListener("click",async()=>{
  ...
  try{
    ...
    const imgs=await Promise.all(selected.map(id=>imgToBase64(CLOTHES.find(c=>c.id===id).src)));
    ...
    if(!gen.ok){ const t=await gen.text(); throw new Error("Generate 실패: "+t); }
...
async function poll(id,n){
  ...
  if(d.task_status==="succeed"){
    box.innerHTML=`<img src="${d.image_url}" style="width:100%">`;
    reset(); return;
  }
