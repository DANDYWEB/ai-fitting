...
let selected = [];
let cat      = "all";
let lastImgUrl = "";                        // ★ 결과 이미지 URL 저장

/* YouTube 대기 영상 (사운드 ON · 컨트롤 없음 · 루프) */
...
function fileToBase64Compressed(f){
  return new Promise(res=>{
    const img=new Image();
    img.onload=()=>{const MAX=1270,scale=Math.min(1,MAX/Math.max(img.width,img.height));
      const c=document.createElement("canvas");
      c.width=Math.round(img.width*scale);c.height=Math.round(img.height*scale);
      c.getContext("2d").drawImage(img,0,0,c.width,c.height);
      const mime=f.type||"image/jpeg";const q=mime==="image/jpeg"?0.85:undefined;
      c.toBlob(b=>{
        const fr=new FileReader();
...
btn.addEventListener("click",async()=>{
  ...
  downloadBtn.hidden=true;downloadBtn.disabled=true;lastImgUrl="";
