...
/* YouTube 대기 영상: 소리 ON, 컨트롤 제거, 무한 반복 */
...
btn.addEventListener("click", async()=>{
  ...
  box.innerHTML = YT_IFRAME;      // 대기 영상
  downloadBtn.hidden = true;      // 다운로드 버튼 숨김
  ...
  try{
    ...
    // ② 의류 하나 또는 두 개
    const clothList = await Promise.all(
      selected.map(id => imgToBase64(CLOTHES.find(c=>c.id===id).src))
    );
    const cloth = clothList.length === 2 ? await mergeTwo(clothList[0], clothList[1]) : clothList[0];
