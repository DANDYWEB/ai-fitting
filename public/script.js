...
/* YouTube 대기 영상 (사운드 ON · 컨트롤 없음 · 무한 루프) */
...
file.addEventListener("change", () => {
  ...
  if (f) {
    ...
    fr.onload = e => {
      prev.innerHTML = `<img src="${e.target.result}" style="width:100%">`;
    };
    fr.readAsDataURL(f);
  }
  updateBtn();
...
/* ──────────────────────── Generate 버튼 활성화 제어 ─────────────────────── */
function updateBtn() {
  btn.disabled = !(file.files[0] && selected.length > 0);
}
